...
-- 9. Top customer by sales in each city
SELECT 
	customerNumber
    ,customerName
    ,city
...
SELECT 
    ...
    ,ROW_NUMBER() OVER 
        ...
        AS rnk
FROM customers c
JOIN orders o ON c.customerNumber = o.orderNumber
JOIN orderdetails od ON o.orderNumber = od.orderNumber
GROUP BY
	c.customerNumber
    ,c.customerName
...
SELECT customerNumber, customerName, city, totalSales
FROM (
    SELECT c.customerNumber, c.customerName, c.city,
           SUM(od.quantityOrdered * od.priceEach) AS totalSales,
           ROW_NUMBER() OVER (PARTITION BY c.city ORDER BY SUM(od.quantityOrdered * od.priceEach) DESC) AS rn
    FROM customers c
    JOIN orders o ON c.customerNumber = o.customerNumber
    JOIN orderdetails od ON o.orderNumber = od.orderNumber
...
SELECT customerNumber, customerName, city, totalSales
FROM (
    SELECT c.customerNumber, c.customerName, TRIM(c.city) AS city,
           SUM(od.quantityOrdered * od.priceEach) AS totalSales,
           ROW_NUMBER() OVER (PARTITION BY TRIM(c.city) ORDER BY SUM(od.quantityOrdered * od.priceEach) DESC) AS rn
    FROM customers c
    JOIN orders o ON c.customerNumber = o.customerNumber
    JOIN orderdetails od ON o.orderNumber = od.orderNumber
    GROUP BY c.customerNumber, c.customerName, TRIM(c.city)
) AS ranked
WHERE rn = 1;
